...
<template>
  ...
  <ul>
    ...
    <li v-for="item in carrello.value.items" :key="item.value">
      ...
      <button v-if="item.productId?.name !== undefined" class="btn2"
        @click="eliminaCarrello(item.productId._id)">Elimina il prodotto dal carrello</button>
      <button v-if="item.productId?.name === undefined" class="btn2"
        @click="svuotaCarrello(); $router.push('/local_shop_advisor_Hosting/catalogo')">Svuota il carrello</button>
    </li>
  ...
  <button class="btn2" :disabled="carrello.value.subTotal == 0"
    @click="aggiungiOrdine(); $router.push('/local_shop_advisor_Hosting/ordini')">Checkout</button>
</template>